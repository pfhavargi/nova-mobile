/* ============================================================
   HOMEPAGE LOGIC
   Fetches the hero_banner and product entries from Contentstack
   (or mock data) and renders them into index.html
   ============================================================ */
...
function renderHero(el, hero) {
  el.innerHTML = `
    ...
    <h1>${hero.title}</h1>
    <p>${hero.subtitle}</p>
    <a class="btn" href="${hero.button_url || "#catalog"}">${hero.button_text || "Shop Now"}</a>
  `;
  if (hero.background_image && hero.background_image.url) {
    document.querySelector(".hero").style.backgroundImage =
      `linear-gradient(180deg, rgba(20,22,26,0.75), rgba(20,22,26,0.95)), url(${hero.background_image.url})`;
